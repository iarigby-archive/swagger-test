...
export type Pick_User.email - or - name - or - phoneNumbers_ = {
    'email': string;
    'name': string;
    'phoneNumbers': Array < string >
    ;
...
export type UserCreationParams = Pick_User.email - or - name - or - phoneNumbers_;
...
export class Test {
    ...
    GetUser(parameters: {
        'userId': number,
        'name' ? : string,
    } & CommonRequestOptions): Promise < ResponseWithBody < 200, User >> {
        const domain = parameters.$domain ? parameters.$domain : this.domain;
        let path = '/users/{userId}';
        if (parameters.$path) {
            path = (typeof(parameters.$path) === 'function') ? parameters.$path(path) : parameters.$path;
    ...
    CreateUser(parameters: {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (parameters['requestBody'] === undefined) {
                reject(new Error('Missing required  parameter: requestBody'));
                return;
            }
